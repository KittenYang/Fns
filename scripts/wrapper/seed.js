...
async function main(a) {
    ...
    await (await NameWrapper.setSubnodeOwnerAndWrap(namehash(`${name}.fns`), 'sub1', firstAddress, CAN_DO_EVERYTHING)).wait()
    await (await NameWrapper.setSubnodeOwnerAndWrap(namehash(`${name}.fns`), 'sub2', firstAddress, CAN_DO_EVERYTHING)).wait()
    await (await NameWrapper.setResolver(namehash(`sub2.${name}.fns`), resolverAddress)).wait()
    await (await Resolver.setText(namehash(`sub2.${name}.fns`), 'domains.fnx.nft.image', 'https://i.imgur.com/JcZESMp.png')).wait()
    console.log(`Wrapped NFT for sub2.${name}.fns is available at ${getOpenSeaUrl(NameWrapper.address, namehash(`sub2.${name}.fns`))}`)
    await (await NameWrapper.burnFuses(namehash(`sub2.${name}.fns`),CANNOT_UNWRAP)).wait()
    await (await NameWrapper.burnFuses(namehash(`sub2.${name}.fns`),CANNOT_SET_RESOLVER)).wait()
    await (await NameWrapper.unwrap(namehash(`${name}.fns`), labelhash('sub1'), firstAddress)).wait()
  }
